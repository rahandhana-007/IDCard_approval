...
(async () => {
  ...
  check('B: panduan cetak minimum tampil di panel user', /minimal ≈/.test($B('printHint').textContent) && /mm/.test($B('printHint').textContent), $B('printHint').textContent.slice(0, 90));
  ...
  check('B: gambar tanda tangan di hasil TIDAK dipaksa latar putih', !/background:#fff/.test($B('vResult').innerHTML));
