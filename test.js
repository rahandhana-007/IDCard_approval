...
(async () => {
  ...
  $('qrPos').value = 'tl'; $('qrPos').dispatchEvent(new winA.Event('change'));
  $('qrCapSize').value = '30'; $('qrCapSize').dispatchEvent(new winA.Event('input'));
  await sleep(150);
  ...
  check('A: approval menyimpan tata letak manager (_layout)', !!row1.card_data._layout && row1.card_data._layout.pos === 'tl' && row1.card_data._layout.cap === '30');
  ...
  check('B: tata letak manager diikuti (pos=tl, caption=30)', $B('qrPos').value === 'tl' && $B('qrCapSize').value === '30', $B('qrPos').value + '/' + $B('qrCapSize').value);
  ...
  check('A: approval menyimpan tanda tangan basah (_sig) di server', row3.status === 'disetujui' && !!(row3.card_data || {})._sig && !('ss' in JSON.parse(row3.payload)));
  await DB.renderMyRequests();
  const dl3 = [...docB.querySelectorAll('#myReqBody [data-dl]')].find(b => b.dataset.dl === row3.id);
  dl3.click(); await sleep(1100);
  check('B: tanda tangan basah direkonstruksi dari server (777 byte)', !!KSB.state.sigImg && KSB.state.sigImg.bytes && KSB.state.sigImg.bytes.length === 777);
